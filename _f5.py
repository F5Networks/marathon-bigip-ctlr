...
def log_list(prefix, list_to_log):
    """Helper function to log a list.

    Dump a list to the logger, skip if the list is empty

    Args:
        prefix: The prefix string to describe what's being logged
        list_to_log: The list being logged
    """
    if list_to_log:
        logger.debug(prefix + ': %s', (', '.join(list_to_log)))
...
class CloudBigIP(BigIP):
    ...
    def _apply_config(self, config):
        ...
        for partition in unique_partitions:
            ...
            log_list('iApps to update', iapp_intersect)
            ...
            log_list('f5_pool_list', f5_pool_list)
            log_list('f5_virtual_list', f5_virtual_list)
            log_list('f5_healthcheck_list', f5_healthcheck_list)
            log_list('marathon_pool_list', marathon_pool_list)
            log_list('marathon_virtual_list', marathon_virtual_list)

            # virtual delete
            virt_delete = list_diff(f5_virtual_list, marathon_virtual_list)
            log_list('Virtual Servers to delete', virt_delete)
            for virt in virt_delete:
                self.virtual_delete(partition, virt)

            # pool delete
            pool_delete_list = list_diff(f5_pool_list, marathon_pool_list)
            log_list('Pools to delete', pool_delete_list)
            ...
            health_delete = list_diff(f5_healthcheck_list,
                                      marathon_healthcheck_list)
            log_list('Healthchecks to delete', health_delete)
            ...
            healthcheck_add = list_diff(marathon_healthcheck_list,
                                        f5_healthcheck_list)
            log_list('Healthchecks to add', healthcheck_add)
            ...
            healthcheck_intersect = list_intersect(marathon_virtual_list,
                                                   f5_healthcheck_list)
            log_list('Healthchecks to update', healthcheck_intersect)
            ...
            log_list('Pools to update', pool_intersect)
            ...
            virt_intersect = list_intersect(marathon_virtual_list,
                                            f5_virtual_list)
            log_list('Virtual Servers to update', virt_intersect)
            ...
            for pool in list(set(pool_add + pool_intersect)):
                ...
                log_list('Pool members to delete', member_delete_list)
                ...
                log_list('Pool members to add', member_add)
                ...
                member_update_list = list_intersect(marathon_member_list,
                                                    f5_member_list)
                log_list('Pool members to update', member_update_list)
